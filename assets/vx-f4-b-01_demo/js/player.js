// Original source on https://github.com/binarymind/multitrackHTMLPlayer
//--------------------------------------------------------------------
//FFT CODE
//--------------------------------------------------------------------
...
generateHierarchy = function(container) {
	...
	myLis.each(function(){
		var that = $(this);
		that.wrap('<ul class="expandable opened"></ul>');
...
generateHtml = function(container) {
	...
	audioTags.each(function(i){
		...
		var randomColor = '01bbf3';
		if (i==0) {
			var add_attributes = 'id= "'+containerID+'__mix_track" style="display: none;"';
			var track_class = "track";
			var mute = "";
		} else {
			var add_attributes = "";
			var track_class = "track mute ";
			var mute = "active";
		}
		containerTracks.append('<li class="'+track_class+'" '+add_attributes+'><ul class="control"><li class="mute '+mute+'"><a href="javascript://"><span></span></a></li></ul><span class="status"></span><audio index="'+i+'" container="'+containerID+'" color="'+randomColor+'" preload="auto" '+srcAttr+'="'+$(this).attr("url")+'"></audio><span class="track-name">'+$(this).attr("name")+'</span>'+($.browser.mozilla ? '<span class="color"></span>':"")+'</li>');
...
initPlayer = function(containerID) {
	...
	container.on("ready", function(){
		$(this).addClass("ready");
		players[containerID].tracks[1].volume = 0;  // SE added this
	});
	...
	container.on('click', '.tracks .track:not(.locked):not(.solo) .mute:not(.active) > a', function() {
		...
		//unmute mix (SE)
		var mix_track = document.getElementById(containerID+'__mix_track');
		$(mix_track).closest('.track').removeClass("mute");

		var mute_active = $(mix_track).find('li:first')[0];
		$(mute_active).removeClass("active");

		mix_track = $(mix_track).closest('.track').find('audio:first')[0];
		mix_track.volume = 1;
	});
	...
	container.on('click', '.tracks .track:not(.locked):not(.solo) .mute.active > a', function() {
		...
		//mute mix (SE)
		var mix_track = document.getElementById(containerID+'__mix_track');
		$(mix_track).closest('.track').addClass("mute");

		var mute_active = $(mix_track).find('li:first')[0];
		$(mute_active).addClass("active");
